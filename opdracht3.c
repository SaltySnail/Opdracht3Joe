...
void process_keugel(player *player, keugel *keugel, float angle, mouse *mouse);
...
// This function has changed because mouse movement was added:
void update_player(player *tha_playa, mouse *tha_mouse, keugel *keugel);
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		// # Applying Game Logic #
		// Also takes the mouse movement into account:
		update_player(&blorp, &mousepointer, &keugel);
		...
		if (keugel.life)
		{
			blit_angled(keugel.txtr_kogel, keugel.x, keugel.y, keugel.angle);
			--keugel.life;
		}
...
void update_player(player *tha_playa, mouse *tha_mouse, keugel *keugel)
{
	process_keugel(tha_playa, keugel, tha_playa->angle, tha_mouse);
	if (tha_playa->up)
	{
